...
float3 EvaluateAmbiance(TextureCube lysBurleyCube, float3 vN, float3 VNUnit, float3 toEye, float perceptualRoughness, float ao, float3 dfcol, float3 spccol)
{
    ...
    float3 specRad = lysBurleyCube.SampleLevel(samplerState, vR, mipLevel).xyz;
    float3 diffRad = lysBurleyCube.SampleLevel(samplerState, vN, (float) (nrBrdMips - 1)).xyz;
...
float3 EvaluateDirectionalLight(float3 albedoColor, float3 specularColor, float3 normal, float roughness,
                             float3 lightColor, float3 lightDir, float3 viewDir)
{
    ...
    // Calculate alpha parameter for GGX
    float alpha = max(0.001f, roughness * roughness + 0.005f);
    float alphaSqr = alpha * alpha;
    
    // Calculate D: Normal Distribution Function (GGX/Trowbridge-Reitz)
    float denomD = (NdH * NdH * (alphaSqr - 1.0) + 1.0);
    float D = alphaSqr / (PI * denomD * denomD);
    
    // Calculate G: Geometry Function (Schlick-GGX)
    float k = alpha / 2.0;
    float G = (NdV / (NdV * (1.0 - k) + k)) * (NdL / (NdL * (1.0 - k) + k));
    
    // Calculate F: Fresnel (Schlick)
    float3 F = specularColor + (1.0 - specularColor) * pow(1.0 - VdH, 5.0f);
    
    // Calculate specular term
    float3 specular = (D * G * F) / max(4.0 * NdV * NdL, 0.001);
    
    // Apply combined dampening
    float specLuminance = dot(specular, float3(0.3, 0.59, 0.11));
    float viewAngleFactor = pow(NdV, 0.5);
    float dampening = 1.0 / (1.0 + max(0, specLuminance - 2.0) / 5.0) * lerp(0.5, 1.0, viewAngleFactor);
    
    // Combine diffuse and dampened specular
    float3 diffuse = albedoColor / PI;
    return lightColor * NdL * (diffuse * (1.0 - F) + specular * dampening);
}
